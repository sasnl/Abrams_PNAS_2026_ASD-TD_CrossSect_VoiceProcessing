%% ROI GLM control: acoustics-adjusted Age×Group (LEAN SI table with fixed decimals)
clear; clc;

%% ---- Paths (base: /Users/daa/Documents/Scratch/TD_ASD_CrossSectional)
base_dir   = '/Users/daa/Documents/Scratch/TD_ASD_CrossSectional';
project    = 'ASD_TD_MothersVoice_Recordings';

roi_dir    = fullfile(base_dir, project, 'Data',    'ROI_Sig_Levels/gPPI/Right_pSTS_Seed');
...
td_roi_sig_fname  = fullfile(roi_dir, 'TDonly_n40_gPPI_SigLev_MothOth_NEG_R_pSTS_Age_Int.mat');
asd_roi_sig_fname = fullfile(roi_dir, 'ASDonly_n39_gPPI_SigLev_MothOth_NEG_R_pSTS_Age_Int.mat');
...
out_csv      = fullfile(out_dir, 'gPPI_R_MotherOth_AgeXGroup_AcousticsControl_ROI_LEAN.csv');      % formatted (SI-ready)
out_csv_raw  = fullfile(out_dir, 'gPPI_R_MotherOth_AgeXGroup_AcousticsControl_ROI_LEAN_raw.csv');  % numeric
...
% ---------- Save RAW (numeric) ----------
Tout_raw = sortrows(Tout, 'p');
writetable(Tout_raw, out_csv_raw);

% ---------- Build SI-FORMATTED table (fixed decimals, p/q with <0.001) ----------
nrow = height(Tout_raw);
ROI = Tout_raw.ROI;

N_str  = arrayfun(@(x) sprintf('%d', x), Tout_raw.N, 'UniformOutput', false);
df_str = arrayfun(@(x) sprintf('%d', x), Tout_raw.df, 'UniformOutput', false);

beta_str   = arrayfun(@(x) sprintf('%.3f', x), Tout_raw.beta_AgeXGroup, 'UniformOutput', false);
ci_lo_str  = arrayfun(@(x) sprintf('%.3f', x), Tout_raw.CI95_low,       'UniformOutput', false);
ci_hi_str  = arrayfun(@(x) sprintf('%.3f', x), Tout_raw.CI95_high,      'UniformOutput', false);
t_str      = arrayfun(@(x) sprintf('%.2f', x), Tout_raw.t,              'UniformOutput', false);
pr2_str    = arrayfun(@(x) sprintf('%.3f', x), Tout_raw.partial_R2,     'UniformOutput', false);
f_str      = arrayfun(@(x) sprintf('%.2f', x), Tout_raw.cohen_f,        'UniformOutput', false);

% p and q with "< 0.001" rule
p_str = cell(nrow,1); q_str = cell(nrow,1);
for i = 1:nrow
    if Tout_raw.p(i) < 0.001, p_str{i} = '<0.001'; else, p_str{i} = sprintf('%.3f', Tout_raw.p(i)); end
    if Tout_raw.q(i) < 0.001, q_str{i} = '<0.001'; else, q_str{i} = sprintf('%.3f', Tout_raw.q(i)); end
end

Tsi = table(ROI, N_str, beta_str, ci_lo_str, ci_hi_str, t_str, df_str, p_str, q_str, pr2_str, f_str, ...
    'VariableNames', {'ROI','N','beta_AgeXGroup','CI95_low','CI95_high','t','df','p','q','partial_R2','cohen_f'});

% Sort by numeric p
[~, order] = sort(Tout_raw.p);
Tsi = Tsi(order, :);

% Save SI-formatted table
writetable(Tsi, out_csv);
fprintf('Wrote SI-formatted (fixed-decimal) table to:\n  %s\n', out_csv);
fprintf('Wrote numeric raw table to:\n  %s\n', out_csv_raw);
