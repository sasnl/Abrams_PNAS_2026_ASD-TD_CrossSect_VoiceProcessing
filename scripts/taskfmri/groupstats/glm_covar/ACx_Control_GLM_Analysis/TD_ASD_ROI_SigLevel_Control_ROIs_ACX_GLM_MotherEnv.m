...
%   - TD_Within_Age.csv              (β per year, t, df, p, partial η²)  -- contrast betas as Y
%   - ASD_Within_Age.csv             (β per year, t, df, p, partial η²) -- contrast betas as Y
%
% Notes:
%   * β units are "contrast units/year". If your contrast betas are % signal change,
%     interpret β as "%/year".
%   * Rounding: β=4 dec, t=2, p=3, partial η²=3. dfs are integers.

close all; clear; clc

%% ======== Paths: ASD ========
asd_age_path = '/Users/daa/Documents/Scratch/TD_ASD_CrossSectional/Data/ASD_selected_age_scan_n39_ADOS.txt';
asd_mat_path = '/Users/daa/Documents/Scratch/TD_ASD_CrossSectional/GLM_Results/TD_ASD_ROI_SigLevel_GLM_n40_n39/Data/Control_ROIs_ACx/ASDonly_n39_Control_ACx_MothEnv.mat';

%% ======== Paths: TD ========
td_age_path  = '/Users/daa/Documents/Scratch/TD_ASD_CrossSectional/Data/TD_selected_age_scan.txt';
td_mat_path  = '/Users/daa/Documents/Scratch/TD_ASD_CrossSectional/GLM_Results/TD_ASD_ROI_SigLevel_GLM_n40_n39/Data/Control_ROIs_ACx/TDonly_n40_Control_ACx_MothEnv.mat';

%% ======== Output directory ========
out_dir = '/Users/daa/Documents/Scratch/TD_ASD_CrossSectional/GLM_Results/TD_ASD_ROI_SigLevel_GLM_n40_n39/Results/Control_ROIs_ACx/Mother_Env';
...
%% ======== (B) ASD-only GLM: ROI ~ Age_c (contrast betas as Y) ========
Age_c_ASD = Age_ASD - mean(Age_ASD,'omitnan');   % center within ASD
ASD_within = local_within_table(roi_asd, Age_c_ASD, roi_names);
writetable(ASD_within, fullfile(out_dir, 'ASD_Within_Age.csv'));

%% ======== (C) TD-only GLM: ROI ~ Age_c (contrast betas as Y) ========
Age_c_TD = Age_TD - mean(Age_TD,'omitnan');      % center within TD
...
% For simple regression Y ~ Age_c (per ROI) using contrast betas as Y:
%   β  = slope for Age_c (contrast units/year)
%   t  = t-stat of Age_c coefficient
%   df = DFE (n - 2)
%   p  = p-value for Age_c
%   partial η² = t^2 / (t^2 + df)
    nR = size(Ymat,2);
    beta  = nan(nR,1);
    tstat = nan(nR,1);
    pval  = nan(nR,1);
    ...
    for rr = 1:nR
        y = Ymat(:,rr);
        Ttmp = table(Age_c_vec, y, 'VariableNames', {'Age_c','Y'});
        mdl  = fitlm(Ttmp, 'Y ~ Age_c');
        c    = mdl.Coefficients;
        % row name for Age_c can be exactly 'Age_c'
