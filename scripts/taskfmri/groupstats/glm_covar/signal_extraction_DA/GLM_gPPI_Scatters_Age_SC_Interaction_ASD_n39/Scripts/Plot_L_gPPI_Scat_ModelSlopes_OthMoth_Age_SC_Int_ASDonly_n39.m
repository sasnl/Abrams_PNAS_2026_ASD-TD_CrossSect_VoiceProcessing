%% ASD-only gPPI Age x Social Communication (SC) follow-ups (Left pSTS seed; Option A)
% - Model uses ADOS-SA (higher = worse) exactly as-is
% - All outputs are Ability-based (no impairment-labeled results in CSV)
%   Ability 25th  ≡ ADOS-SA 75th (low ability)
%   Ability 75th  ≡ ADOS-SA 25th (high ability)
% - Saves figures to Scatters/gPPI_Scatters and CSV to Results
...
mat_path = '/Users/daa/Documents/Scratch/TD_ASD_CrossSectional/GLM_gPPI_Scatters_Age_SC_Interaction_ASD_n39/Data/ASDonly_n39_gPPI_SigLev_MothOth_L_pSTS_Age_SC_Int_pos.mat';
...
% >>> FLIP CONTRAST: [Mother’s voice − NF voices] -> [NF voices − Mother’s voice]
roi_con = -roi_con;
% <<< FLIP CONTRAST
...
%% ===== Style (for plots) =====
% Colors mapped to Ability levels:
%   CLR.lo = Low ability (Ability 25th ≡ ADOS-SA 75th)
%   CLR.hi = High ability (Ability 75th ≡ ADOS-SA 25th)
CLR.lo  = [0.95 0.25 0.15];   % red-orange (Low ability)
CLR.hi  = [0.00 0.60 1.00];   % blue-cyan (High ability)
CLR.pts = [0.7 0.7 0.7];      % points
MSZ  = 70;  LW = 2.75;  EDLW = 0.6;  ALPH = 0.20;

%% ===== Prep accumulator =====
rows = [];   % set width on first row

%% ===== Loop over ROIs (one figure per ROI) =====
...
for r = 1:nROIs
    ...
    % Fit model with interaction (ADOS-SA space)
    T = table(Age_c, SC_c, y, 'VariableNames', {'Age_c','SC_c','gPPI'});
    mdl = fitlm(T, 'gPPI ~ Age_c * SC_c');   % main effects + interaction

    % -------- Compute Ability percentiles & centers (labels only) --------
    sc_p25_ADOS  = prctile(SC, 25);   % (internal only; higher = worse)
    sc_p75_ADOS  = prctile(SC, 75);
    sc25_c_ADOS  = sc_p25_ADOS - SC_mu;
    sc75_c_ADOS  = sc_p75_ADOS - SC_mu;

    % Ability 25th  ≡ ADOS-SA 75th ; Ability 75th ≡ ADOS-SA 25th
    ...
    % Prediction tables at Ability 25th (low) and 75th (high)
    ...
    % ===== Simple slopes (compute internally; output only Ability slopes) =====
    ...
    % Slopes at ADOS-SA percentiles (internal to derive Ability mapping)
    ...
    % Ability-labeled slopes (reported)
    ...
    % ===== Model fit + ΔR² and nested test =====
    mdl_red = fitlm(T, 'gPPI ~ Age_c + SC_c');                     % reduced (no interaction)
    dR2  = mdl.Rsquared.Ordinary - mdl_red.Rsquared.Ordinary;      % ΔR²

    Fchg = NaN; pchg = NaN;
    try
        cmp = compare(mdl_red, mdl);       % preferred when available
        ...
        try
            A2 = anova(mdl_red, mdl);      % two-model ANOVA
    ...
    % ===== Collect row for CSV (Ability-only outputs) =====
    % Main effects for completeness
    ...
    % Build row explicitly as cells (each entry scalarized)
    vals = num2cell([ ...
    ...
    if isempty(rows)
        rows = cell(0, numel(row));   % initialize width on first pass
    end
    rows(end+1, :) = row;
...
% NOTE: Ability-only columns (no impairment-labeled outputs)
...
% Write CSV (distinct filename for gPPI / Left pSTS seed)
out_csv = fullfile(save_dir_results,'ASD_AgeXSC_gPPI_LeftpSTS_ROI_stats_ABILITYONLY.csv');
writetable(Tout, out_csv);
fprintf('\nWrote CSV to: %s\n', out_csv);

%% ===== Local helpers =====
function x = scalar(xin)
%SCALAR Ensure a numeric scalar; convert [] to NaN to keep row length fixed.
    if isempty(xin)
        x = NaN;
    else
        x = double(xin);
        x = x(1); % in case xin is 1x1 table/array
    end
end
