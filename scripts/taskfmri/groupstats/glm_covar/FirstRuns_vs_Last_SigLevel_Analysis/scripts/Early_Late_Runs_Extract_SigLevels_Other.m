%% early_late_roi_unfamiliar_signal_analysis_multiROI.m
% Extract ROI signal for unfamiliar voices from first-level SPM models
% using the first 3 vs last 3 runs across ASD and TD participants.
%
% Unfamiliar voice signal is defined as the mean of:
%   - Cont1
%   - Cont2
%
% ROIs:
...
subject_output_csv = fullfile(stats_dir, 'ROI_EarlyLate_UnfamiliarSignal.csv');
stats_output_csv   = fullfile(stats_dir, 'ROI_EarlyLate_Unfamiliar_STATS.csv');
...
for i = 1:height(all_tbl)
    ...
    for r = 1:length(roi_names)
        ...
        for k = 1:length(run_ids)
            cont1_val = get_beta(SPM, run_ids(k), 'Cont1', roi);
            cont2_val = get_beta(SPM, run_ids(k), 'Cont2', roi);
            run_vals(k) = mean([cont1_val, cont2_val], 'omitnan');
        end
